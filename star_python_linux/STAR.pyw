...
def Doit(todecode):
	...
	print(str(settings[13]))
	print(settings[12])

	config.set_string('-hmm', path.join(MODELDIR, 'en-us/en-us'))
	if settings[12]==1:
		config.set_string('-lm', path.join(DICTDIR, str(settings[13])+'.lm'))

	else:
		config.set_string('-lm', path.join(MODELDIR, 'en-us/en-us.lm.bin'))

	if settings[12]==1:	
		config.set_string('-dict',path.join(DICTDIR, str(settings[13])+'.dic'))
	else:
